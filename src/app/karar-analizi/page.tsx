...
const SECTIONS: { key: keyof DecisionAnalysisResult; title: string; isComment?: boolean }[] = [
  ...
  { key: 'dayanilanKurallar', title: 'Dayanılan kurallar' },
...
export default function KararAnaliziPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError('')
    try {
      const data = await analyzeDecision(text.trim())
      setResult(data)
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Karar analizi yapılamadı.'
      setError(msg)
    } finally {
      setLoading(false)
    }
        ...
        <form onSubmit={handleSubmit} className="max-w-3xl mx-auto space-y-4 animate-fade-in-up">
          <label className="block">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5 mb-1">
              Mahkeme kararı metnini veya karar özetini yapıştırın. Metin eksikse analizde belirtilir; tarih veya dosya numarası metinde yoksa uydurulmaz.
            </p>
            <textarea
              value={text}
