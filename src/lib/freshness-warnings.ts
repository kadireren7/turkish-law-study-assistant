 ...
 * - Only show warnings when: source verification is older than 12 months, or an amendment was explicitly detected.
 * - If all sources were verified within the last 6 months, do not show any warning.
...
/** Do not show any warning if every source was verified within this many months. */
const RECENT_OK_MONTHS = 6

/** Only warn "old source" if last_checked is older than this many months. */
const OLD_SOURCE_MONTHS = 12
...
const WARNING_OLD_SOURCE = 'Kaynaklar 12 aydan eski; güncel kaynaklarla yeniden doğrulanmalıdır.'
 ...
 * Only returns warnings when: verification older than 12 months, or explicit amendment detected.
 */
export async function getFreshnessWarnings(sources: string[]): Promise<string[]> {
  ...
  // If every source has last_checked within 6 months, do not show any warning.
  const allHaveDate = dates.every((d) => d !== null)
  const allWithin6Months = allHaveDate && dates.every((d) => isWithinMonths(d, RECENT_OK_MONTHS))
  if (allWithin6Months) return []
  ...
  // Only warn "old source" when verification is older than 12 months
