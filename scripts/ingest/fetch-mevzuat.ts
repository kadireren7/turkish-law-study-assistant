#!/usr/bin/env npx tsx
/**
 * Otomatik mevzuat veri çekme: mevzuat.gov.tr'den kanun metinlerini alıp data/core/laws dosyalarına yazar.
...
import { logPipelineEvent, withTiming } from '../lib/pipeline-logger'
...
async function main(): Promise<void> {
  ...
  console.log('Tamamlandi. data/core/laws guncellendi.')
  logPipelineEvent('mevzuat', 'write', 'batch complete')
}

withTiming('mevzuat-fetch', main).catch((err) => {
  console.error(err)
  process.exit(1)
})
