...
function buildDecisionsMarkdown(inputDecisions: InputDecision[], runDate: string): string {
  ...
  if (inputDecisions.length === 0) {
    lines.push('Henüz karar özeti eklenmemiş. `data/derived/updates/input/decisions.json` dosyasına ekleyebilirsiniz.', '')
  } else {
...
async function main(): Promise<void> {
  ...
  console.log('  History: data/derived/updates/history/ (previous summaries kept before overwrite)')
}
